...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (data.type === 'JOIN_ROOM') return handleJoinRoom(ws, data.payload.roomId);

      if (data.type === 'ADD_NODE') return handleAddNode(ws, data.payload.node);

      if (data.type === 'REMOVE_NODE') return handleRemoveNode(ws, data.payload.nodeId);

      if (data.type === 'UPDATE_NODE_DATA')
        return handleUpdateNodeData(ws, data.payload.nodeId, data.payload.newData);
